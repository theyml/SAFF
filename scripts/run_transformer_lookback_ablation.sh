...
DATA_DIR="$ROOT/data"
RESULT_DIR="${RESULT_DIR:-$ROOT/results_transformer_lookback_ablation}"
LOG_DIR="${LOG_DIR:-$ROOT/logs_transformer_lookback_ablation}"

PYTHON_BIN="${PYTHON_BIN:-python}"
CUDA_DEVICES="${CUDA_DEVICES:-0,1,2,3,4,5,6,7}"
MAX_JOBS="${MAX_JOBS:-8}"
LOOKBACK_DAYS="${LOOKBACK_DAYS:-3,7,14,30,60}"
ITRS="${ITRS:-3}"
...
# Small mechanism-validation sweep for temporal relevance.
...
news_file_for_ticker() {
  case "$1" in
    GOOGL_PRECOVID)
      first_existing GOOGL_title_finbert_emb.csv
      ;;
    NFLX_PRECOVID)
      first_existing NFLX_title_finbert_emb_precovid.csv NFLX_title_finbert_emb.csv
      ;;
    *)
...
IFS=',' read -r -a LOOKBACKS <<< "$LOOKBACK_DAYS"
JOB_INDEX=0
...
run_one() {
  ...
  echo
  echo "===== lookback=$lookback $mode | $ticker | TransformerNewsDecay | sl=$sl pl=$pl | CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-unset} ====="
  ...
  "$PYTHON_BIN" decay_run.py \
    ...
    --des "TransformerNewsDecay_${mode}_lookback${lookback}_g" \
    | tee "$LOG_DIR/${stem}_sl${sl}_pl${pl}_lookback${lookback}_${mode}.log"
}
...
for row in "${ROWS[@]}"; do
  ...
  for lookback in "${LOOKBACKS[@]}"; do
    ...
    [[ -z "$lookback" ]] && continue
    launch "$ticker" "$sl" "$ll" "$pl" "$lookback" plain
    launch "$ticker" "$sl" "$ll" "$pl" "$lookback" decay
  done
done
