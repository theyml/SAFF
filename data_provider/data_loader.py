...
class Dataset_Custom(Dataset):
    def __init__(
        ...
        self.scaler_mode = getattr(args, 'scaler_mode', 'window')
        ...
        assert self.scaler_mode in ['window', 'global'], f'Error: scaler_mode {self.scaler_mode!r} is not supported.'
    ...
    def __read_data__(self):
        ...
        if self.args.percent < 100:
            train_length = train_border2 - self.seq_len - self.pred_len - train_border1
            train_border1 = train_length * (100 - self.args.percent) // 100 + train_border1
        ...
        if self.scale and self.scaler_mode == 'global':
            self.global_scaler = StandardScaler().fit(
                df_data.iloc[train_border1:train_border2].values
            )
    ...
    def __getitem__(self, index):
        ...
        if self.scale:
            if self.scaler_mode == 'global' and self.global_scaler is not None:
